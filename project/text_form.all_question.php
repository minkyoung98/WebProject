...
$sql_before = "SELECT * FROM all_question WHERE seq = $seq-1";
$result_before = $conn->query($sql_before);

$sql = "SELECT * FROM all_question WHERE seq = $seq";
$result = $conn->query($sql);

$sql_next = "SELECT * FROM all_question WHERE seq = $seq+1";
...
<script>
    ...
    $(document).on('click', '#table_navi #navi_next', function(){
        if($('#table_navi').find('td:eq(1)').text() != ''){
            location.href = 'text_form.all_question.php?seq='+ (seq+1);
        }
        else{
            alert("마지막 글입니다.");
        }     
    ...
    $(document).on('click', '#index' , function(){
        location.href = 'editor.all_question.php';
    });
