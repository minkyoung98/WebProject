...
$sql_before = "SELECT * FROM order_inquiry WHERE seq = $seq-1";
$result_before = $conn->query($sql_before);

$sql = "SELECT * FROM order_inquiry WHERE seq = $seq";
$result = $conn->query($sql);

$sql_next = "SELECT * FROM order_inquiry WHERE seq = $seq+1";
        ...
        <main role="main" class="main">
            ...
            <hr>
                ...
                <button id="index" type="button" class="btn btn-secondary">목록으로</button>
                    <input type="hidden" name="category" id="category" value="주문문의">
...
<script>
    ...
    $(document).on('click', '#table_navi #navi_next', function(){
        if($('#table_navi').find('td:eq(1)').text() != ''){
            location.href = 'text_form.order_inquiry.php?seq='+ (seq+1);
        }
        else{
            alert("마지막 글입니다.");
        }     
    ...
    $(document).on('click', '#index' , function(){
        location.href = 'editor.order_inquiry.php';
    });
