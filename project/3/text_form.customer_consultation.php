...
$sql_before = "SELECT * FROM customer_consultation WHERE seq = $seq-1";
$result_before = $conn->query($sql_before);

$sql = "SELECT * FROM customer_consultation WHERE seq = $seq";
$result = $conn->query($sql);

$sql_next = "SELECT * FROM customer_consultation WHERE seq = $seq+1";
        ...
        <main role="main" class="main">
            ...
            <hr>
                ...
                <form id="cate" action = "../editor.update.php?seq=<?php echo $seq ?>" method="post">
                    ...
                    <input type="hidden" name="category" id="category" value="고객상담">
...
<script>
    ...
    $(document).on('click', '#table_navi #navi_next', function(){
        if($('#table_navi').find('td:eq(1)').text() != ''){
            location.href = 'text_form.customer_consultation.php?seq='+ (seq+1);
        }
        else{
            alert("마지막 글입니다.");
        }     
    ...
    $(document).on('click', '#index' , function(){
        location.href = 'editor.customer_consultation.php';
    });
