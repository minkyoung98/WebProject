...
$sql_before = "SELECT * FROM a_s WHERE seq = $seq-1";
$result_before = $conn->query($sql_before);

$sql = "SELECT * FROM a_s WHERE seq = $seq";
$result = $conn->query($sql);

$sql_next = "SELECT * FROM a_s WHERE seq = $seq+1";
...
<script>
    $(document).ready(function() {
        ...
        $('#sideNav #dropOrder > a:nth-child(6)').css({
            'font-weight': 'bold',
            color: 'salmon'
        });
    });
...
<script>
    ...
    $(document).on('click', '#table_navi #navi_next', function(){
        if($('#table_navi').find('td:eq(1)').text() != ''){
            location.href = 'text_form.a_s.php?seq='+ (seq+1);
        }
        else{
            alert("마지막 글입니다.");
        }     
