...
$sql_before = "SELECT * FROM notice WHERE seq = $seq-1";
                    ...
                    <?php
                        if(isset($_SESSION['id'])){
                            if($_SESSION['id']==$userid){
                                //로그인 후
                                echo '<input type="hidden" name="category" id="category" value="공지사항">
...
<script>
    $(document).ready(function() {
        ...
        $('#sideNav #dropCommunity > a:nth-child(1)').css({
            'font-weight': 'bold',
            color: 'salmon'
        });
    });
...
<script>
    ...
    $(document).on('click', '#table_navi #navi_next', function(){
        if($('#table_navi').find('td:eq(1)').text() != ''){
            location.href = 'text_form.notice.php?seq='+ (seq+1);
        }
        else{
            alert("마지막 글입니다.");
        }     
    ...
    $(document).on('click', '#index' , function(){
        location.href = 'editor.notice.php';
    });
